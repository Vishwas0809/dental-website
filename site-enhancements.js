(function () {
    ...
    function formatCounterValue(el, value) {
        ...
        var formatted = isDecimal ? value.toFixed(1) : String(Math.round(value));
    ...
    function setupCounters() {
        ...
        if (!("IntersectionObserver" in window)) {
            counters.forEach(function (el) {
                el.textContent = formatCounterValue(el, parseFloat(el.getAttribute("data-counter")));
            });
            return;
        }
    ...
    function setupMobileStickyCta() {
        function ensureBar() {
            var existing = document.querySelector(".mobile-sticky-cta");
            ...
            bar.innerHTML =
                '<a class="btn btn-primary" href="contact.html" data-track="mobile_sticky_book"><i class="fa-solid fa-calendar-check" aria-hidden="true"></i> Book Visit</a>' +
                '<a class="btn btn-whatsapp" href="https://wa.me/' + whatsappNumber + '" target="_blank" rel="noopener" data-track="mobile_sticky_whatsapp"><i class="fa-brands fa-whatsapp" aria-hidden="true"></i> WhatsApp</a>';
            document.body.appendChild(bar);
            document.body.classList.add("has-mobile-sticky-cta");
            bar.querySelectorAll("[data-track]").forEach(function (link) {
                link.addEventListener("click", function () {
                    trackEvent(link.getAttribute("data-track"), { page: window.location.pathname });
                });
            });
        }
